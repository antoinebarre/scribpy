"""PlantUML block rendering transform for the assembly pipeline."""
...
from scribpy.core.plantuml.renderer import PlantUmlRenderer
...
def render_plantuml_blocks(
    ...
    """Replace PlantUML fenced blocks with PNG image references.

    Each ````plantuml` block is rendered to a PNG file in *generated_dir*
    and replaced by a ``![diagram](assets/generated/<hash>.png)`` reference.
    ...
    Args:
        content: Markdown source text containing zero or more PlantUML blocks.
    ...
    Returns:
        Markdown source text with PlantUML blocks replaced by image references.
    """
    ...
    return _PLANTUML_BLOCK.sub(_replace, content)
