...
def test_heading_hierarchy_rule_reports_jump(tmp_path: Path) -> None:
    ...
    result = run_lint_rules(
        _context(tmp_path, document), (HeadingHierarchyRule(),)
    )
...
def test_broken_internal_link_rule_reports_missing_anchor(
    tmp_path: Path,
) -> None:
    current = _document(
...
def test_broken_internal_link_rule_reports_missing_document(
    tmp_path: Path,
) -> None:
    current = _document(
        tmp_path, "index.md", "# Home\n\n[Missing](ghost.md)\n"
...
def test_broken_internal_link_rule_accepts_external_link(
    tmp_path: Path,
) -> None:
    document = _document(
    ...
    result = run_lint_rules(
        _context(tmp_path, document), (BrokenInternalLinkRule(),)
    )

    assert result.diagnostics == ()


def test_broken_internal_link_rule_accepts_local_anchor(
    tmp_path: Path,
) -> None:
...
def test_missing_local_asset_rule_reports_missing_file(tmp_path: Path) -> None:
    ...
    result = run_lint_rules(
        _context(tmp_path, document), (MissingLocalAssetRule(),)
    )
...
def test_missing_local_asset_rule_accepts_existing_file(
    tmp_path: Path,
) -> None:
    document = _document(
        tmp_path, "index.md", "# Home\n\n![Logo](assets/logo.png)\n"
...
def test_missing_local_asset_rule_accepts_external_asset(
    tmp_path: Path,
) -> None:
    document = _document(
        tmp_path,
